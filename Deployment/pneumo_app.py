...
def run_the_app():
    ...
    predictor.load_weights("predictor_weights.h5")
    ...
    st.subheader(info)
    info = "This model uses our non-oversampled classifier and the selected predictor."
    st.write(info)
    predictor_size = "Predictor size: " +  convert_bytes('predictor_weights.h5', "MB")
    st.write(predictor_size)
    predictor_size = "Classifier size: " +  convert_bytes('classifier_weights.h5', "MB")
    st.write(predictor_size)
    st.write("Time taken for prediction: ",round(etime-stime,2) ,"seconds.") 
    st.image(pred_mask, use_column_width=False)
    
    #Model2
    classifier_over = buildClassifier()
    classifier_over.load_weights("classifier_oversampled_weights.h5")
    ...
    st.subheader(info)
    info = "This model uses our oversampled classifier and the selected predictor."
    st.write(info)
    predictor_size = "Predictor size: " +  convert_bytes('predictor_weights.h5', "MB")
    st.write(predictor_size)
    predictor_size = "Over sampled classifier size: " +  convert_bytes('classifier_oversampled_weights.h5', "MB")
    st.write(predictor_size)
    st.write("Time taken for prediction: ",round(etime-stime,2) ,"seconds.") 
    st.image(pred_mask, use_column_width=False)
    ...
    st.subheader(info)
    info = "Here we used our non-oversampled classifier and the selected quantized predictor."
    st.write(info)
    predictor_size = "Quantized Predictor size: " +  convert_bytes('predictor.tflite', "MB")
    st.write(predictor_size)
    predictor_size = "Quantized classifier size: " +  convert_bytes('classifier.tflite', "MB")
    st.write(predictor_size)
    st.write("Time taken for prediction: ",round(etime-stime,2) ,"seconds.") 
    st.image(pred_mask, use_column_width=False)
    ...
    etime = time.time()
    pred_mask = tf.keras.preprocessing.image.array_to_img(pred_masks)
    info = "Predicted Mask : 4"
    st.subheader(info)
    info = "Here we used our oversampled classifier and the selected quantized predictor."
    st.write(info)
    predictor_size = "Quantized Predictor size: " +  convert_bytes('predictor.tflite', "MB")
    st.write(predictor_size)
    predictor_size = "Quantized over sampled classifier size: " +  convert_bytes('classifier_over.tflite', "MB")
    st.write(predictor_size)
    st.write("Time taken for prediction: ",round(etime-stime,2) ,"seconds.") 
    st.image(pred_mask, use_column_width=False)
...
def download_file(file_path):
    ...
    info = "Downloading file: " + file_path + "... this may take a while ! Don't stop it!"
